...
function setup() 
{
	...
	//camera overlay to take a picture to add it to the scene
	cameraContentOn();
...
function takeAPicture()
{
	...
	context.getImageData(0,0, camera.width, camera.width);
...
//fill canvases with ctech colors in ctech color order
...
{	
	...
	// not working asychron
	// updateCanvasDataLists();
	// //never ending rekusion for an neverending animation
	// moveContent();
}
...
function updateCanvasDataLists()
{
	for(let i in canvasDataLists)
	{
		canvasDataLists[i][1] = imageList[Math.floor(Math.random() * imageList.length)];
	}
}
